...
#specsdf_air <- left_join(specsdf, airscan, by = "Ramanshift")%>% #code to subtract airscan from each without writing a loop, restore columns and column names
  #mutate(spc = spc.x-spc.y)%>%
  #select(c(filename.x, Ramanshift, spc))%>%
  #rename(filename = filename.x)

ggplot(specsdf)+
  geom_line(aes(x=Ramanshift,y=spc,color=filename))+
  labs(title="Raw raman plot: airscan subtract")+
  theme(legend.position = "none") #This line of code will turn legend off. Too many samples at once and the legend swamps the figure. 
...
#changing fingerprint to 2000 because of spikes
fingerprint<-specsdf[specsdf$Ramanshift>=600 & specsdf$Ramanshift<=1800,] #used to be 200 t0 2100 recommended by Bharat.
stretch<-specsdf[specsdf$Ramanshift>=2600 & specsdf$Ramanshift<=3250,] #2600 to 3250 recommended by Bharat. 
```
...
filenames <- fingerprint_w_EMSC[,1] #save filenames for later
...
fp_baseline <- as.data.frame(cbind(filenames,polyfit_res$baseline))
fp_corrected <- as.data.frame(cbind(filenames,polyfit_res$corrected))
...
```{r}
# How to do this...need a matrix of spectra
filenames <- stretch_w_EMSC[,1] #save filenames for later
...
ggplot(fingerprint_EMSC_mpoly_sg)+
  geom_line(aes(x = Ramanshift, y = spc_sg, color = filenames))+
...
write.csv(x = fingerprint_EMSC_mpoly_sg, file = "preprocessed_fingerprint_1deriv.csv")
# Output file with stretch_EMSC_mpoly_sg data
write.csv(x = stretch_EMSC_mpoly_sg, file = "preprocessed_stretch_1deriv.csv")
