...
#Filter out OSNT, too few samples
maturity_data <- maturity_data%>% 
  filter(HistoMaturity != "OSNT")

maturity_data$HistoMaturity <- factor(maturity_data$HistoMaturity)
...
ggsave("fp_avgspectra.pdf", width = 8, height = 5)
```
...
ggsave("fp2deriv_avgspectra.pdf", width = 8, height = 5)
```
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m1$res$cal$scores[,1], m1$res$cal$scores[,2], color = fp_data_2deriv_w$HistoMaturity))+
    labs(title="Scores",
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m1$res$cal$scores[,1], m1$res$cal$scores[,3], color = fp_data_2deriv_w$HistoMaturity))+
  labs(title="Scores",
       caption="Source: MACE 2017 Pollock",
       x="Comp 1",
       y="Comp 3",
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m1$res$cal$scores[,2], m1$res$cal$scores[,3], color = fp_data_2deriv_w$HistoMaturity))+
  labs(title="Scores",
       caption="Source: MACE 2017 Pollock",
       x="Comp 2",
       y="Comp 3",
...
#try reordering? error bars are showing up weird
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m2$res$cal$scores[,1], m2$res$cal$scores[,2], color = stretch_data_2deriv_w$HistoMaturity))+
  labs(title="Scores",
       caption="Source: MACE 2017 Pollock",
       x="Comp 1",
       y="Comp 2",
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m2$res$cal$scores[,1], m2$res$cal$scores[,3], color = stretch_data_2deriv_w$HistoMaturity))+
  labs(title="Scores",
       caption="Source: MACE 2017 Pollock",
       x="Comp 1",
       y="Comp 3",
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m2$res$cal$scores[,2], m2$res$cal$scores[,3], color = stretch_data_2deriv_w$HistoMaturity))+
  labs(title="Scores",
       caption="Source: MACE 2017 Pollock",
       x="Comp 2",
       y="Comp 3",
...
peak_dat <- fp_data_2deriv%>%
  group_by(barcode, peaknum, HistoMaturity)%>%
  filter(spc_sg_n == max(spc_sg_n))%>%
   ungroup()%>%
  transmute(barcode, peaknum, HistoMaturity, intensity = spc_sg_n, ramanshift = Ramanshift, length, GSI)
...
ggplot()+
  ...
  geom_point(data = subset(peak_dat, barcode == 64392), aes(ramanshift, intensity, group = barcode, color = HistoMaturity))+
  theme_classic()
...
ggplot(peak_dat)+
  ...
  geom_point(aes(ramanshift, intensity, group = barcode, color = HistoMaturity))+
  scale_color_manual(values = oo_colors)+
  theme_classic()
  
```

### Find peak intensities for stretch

```{r}

#Sort peak values into ranges by ramanshift
stretch_data_2deriv$peaknum <- ifelse(stretch_data_2deriv$Ramanshift >= 2850 & stretch_data_2deriv$Ramanshift <= 2890, 14,
         ifelse(stretch_data_2deriv$Ramanshift >= 2917 & stretch_data_2deriv$Ramanshift <= 2953, 15,
                ifelse(stretch_data_2deriv$Ramanshift >= 2957 & stretch_data_2deriv$Ramanshift <= 3007, 16, NA)))

stretch_data_2deriv$peaknum <- as.factor(stretch_data_2deriv$peaknum)
stretch_data_2deriv$barcode <- as.factor(stretch_data_2deriv$barcode)

peak_dat_s <- stretch_data_2deriv%>%
  group_by(barcode, peaknum, HistoMaturity)%>%
  filter(spc_sg_n == max(spc_sg_n))%>%
   ungroup()%>%
  transmute(barcode, peaknum, HistoMaturity, intensity = spc_sg_n, ramanshift = Ramanshift, length, GSI)
...
ggplot()+
  geom_line(data = subset(stretch_data_2deriv, barcode == 64392), aes(x = Ramanshift, y = spc_sg_n, group = filenames))+
  geom_point(data = subset(peak_dat_s, barcode == 64392), aes(ramanshift, intensity, group = barcode, color = HistoMaturity))+
  theme_classic()
...
ggplot(peak_dat_s)+
  geom_line(data = stretch_data_2deriv, aes(x = Ramanshift, y = spc_sg_n, group = filenames))+
  geom_point(aes(ramanshift, intensity, group = barcode, color = HistoMaturity))+
  scale_color_manual(values = oo_colors)+
  theme_classic()
...
peak_alldat <- rbind(peak_dat, peak_dat_s)

## Pivot wider
peakdat_w<-peak_alldat%>%
  tidyr::pivot_wider(.,values_from=c("intensity"),id_cols=c("barcode", "HistoMaturity", "GSI", "length"), names_from="peaknum")

ggplot(peakdat_w)+
  geom_bar(aes(HistoMaturity))+
  theme_classic()
```
A lot of vitellogenic samples. May need to balance this and re-run. Ideal sample size would be events per variable (EPV) of >10. This would be # of parameters * 10 = minimum in a resonse category. de Jong et al. 2019. Balanced data does not seem to improve results. Could show this in a simulation to prove to reviewers. 

### Subsample to make more even distribution
```{r}
#subsample for more even distribution
peakdat_w_temp <- peakdat_w%>%
  filter(HistoMaturity != "DEV" & HistoMaturity !="SNT")

peakdat_w_temp <- peakdat_w_temp%>%
  group_by(HistoMaturity)%>%
  sample_n(20)

peakdat_sub <- rbind(peakdat_w[which(peakdat_w$HistoMaturity == "SNT" | peakdat_w$HistoMaturity == "DEV"),], peakdat_w_temp)
...
peakdat_w <- peakdat_sub%>%
  rename(peak1 = `1`, peak2 = `2`, peak3 = `3`, peak4 = `4`, peak5 = `5`, peak6 = `6`, peak7 = `7`, peak8 = `8`, peak9 = `9`, peak10 = `10`, peak11= `11`, peak12 = `12`, peak13 = `13`, peak14 = `14`, peak15 = `15`, peak16 = `16`)

#mean center all variables - leave out peak 5 and 8for now because not full sample set.

#leaving out peak 14 becuase of such small variation among spectra. 
peakdat_w <- peakdat_w%>%
  mutate(GSI.m = GSI - mean(peakdat_w$GSI), length.m = length - mean(peakdat_w$length), peak1.m = peak1 - mean(peakdat_w$peak1), peak2.m = peak2 - mean(peakdat_w$peak2), peak3.m = peak3 - mean(peakdat_w$peak3), peak4.m = peak4 - mean(peakdat_w$peak4), peak5.m = peak5 - mean(peakdat_w$peak5), peak6.m = peak6 - mean(peakdat_w$peak6), peak7.m = peak7 - mean(peakdat_w$peak7), peak8.m = peak8 - mean(peakdat_w$peak8), peak9.m = peak9 - mean(peakdat_w$peak9), peak10.m = peak10 - mean(peakdat_w$peak10), peak11.m = peak11 - mean(peakdat_w$peak11), peak12.m = peak12 - mean(peakdat_w$peak12), peak13.m = peak13 - mean(peakdat_w$peak13), peak14.m = peak14 - mean(peakdat_w$peak14), peak15.m = peak15 - mean(peakdat_w$peak15), peak16.m = peak16 - mean(peakdat_w$peak16))

peakdat_l <- peakdat_w%>%tidyr::pivot_longer(.,cols=c(23:ncol(peakdat_w)),names_to="peak_num",values_to="peak_diff") 

ggplot(peakdat_l)+
  geom_point(aes(peak_num, peak_diff))

```
Not seeing a lot of support for using stretch region peaks... variation is so small and visually not a lot of separation in peaks either. 


### Run the model
```{r}
peak_moddf <- peakdat_w[,c(2,23:35)] #peaks 1-13 from fingerprint
peak_mat <- model.matrix(HistoMaturity~., peak_moddf)[,-1]
str(peak_mat)
hist_vec <- factor(peak_moddf$HistoMaturity)
# 
...
# plot_14 <- ggplot(subset(RRR_df, peak_num == "peak14.m"))+
#   geom_point(aes(x = RRR_m, y = peak_num, color = maturity), position = position_dodge(width=1), key_glyph = "point")+
#   #xlim(.9, 1.1)+
#   scale_color_manual(values = oo_colors)+
#   guides(color = guide_legend(reverse = TRUE, override.aes = list(size=2)))+
...
#   theme_classic()
# 
# plot_15 <- ggplot(subset(RRR_df, peak_num == "peak15.m"))+
#   geom_point(aes(x = RRR_m, y = peak_num, color = maturity), position = position_dodge(width=1), key_glyph = "point")+
#   #xlim(.9, 1.1)+
#   scale_color_manual(values = oo_colors)+
#   guides(color = guide_legend(reverse = TRUE, override.aes = list(size=2)))+
...
#   theme_classic()
# 
# plot_16 <- ggplot(subset(RRR_df, peak_num == "peak16.m"))+
#   geom_point(aes(x = RRR_m, y = peak_num, color = maturity), position = position_dodge(width=1), key_glyph = "point")+
#   #xlim(.9, 1.1)+
#   scale_color_manual(values = oo_colors)+
#   guides(color = guide_legend(reverse = TRUE, override.aes = list(size=2)))+
...
```{r, warnings = F, fig.width=4, fig.height=10, }
...
#Peak 13
d_peak13 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = mean(peak_moddf$peak4.m),  peak5.m = mean(peak_moddf$peak5.m), peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = peak_moddf$peak13.m, peak14.m = mean(peak_moddf$peak14.m), peak15.m = mean(peak_moddf$peak15.m), peak16.m = mean(peak_moddf$peak16.m))
...
# #Peak 14
# d_peak14 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = mean(peak_moddf$peak4.m),  peak5.m = mean(peak_moddf$peak5.m), peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = mean(peak_moddf$peak13.m), peak14.m = peak_moddf$peak14.m, peak15.m = mean(peak_moddf$peak15.m), peak16.m = mean(peak_moddf$peak16.m))
# 
# pp.peak14 <- as.data.frame(predict(cv, newx = d_peak14, type = "response", s = "lambda.min"))
# pp.peak14 <- cbind(pp.peak14, peak_moddf$peak14.m)
# 
# pp.peak14 <- pp.peak14%>%
#   transmute(peak = "14", `peak_moddf$peak14.m`, IMM.1, ND.1, DEV.1, VIT.1, PSWN.1, SWN.1, PSNT.1, SNT.1)%>%
#   rename(peak_diff = `peak_moddf$peak14.m`, IMM = IMM.1, ND = ND.1, DEV = DEV.1, VIT = VIT.1, PSWN = PSWN.1, SWN = SWN.1, PSNT = PSNT.1, SNT = SNT.1) #peak diff from mean because centered
# 
# #Peak 15
# d_peak15 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = mean(peak_moddf$peak4.m),  peak5.m = mean(peak_moddf$peak5.m), peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = mean(peak_moddf$peak13.m), peak14.m = mean(peak_moddf$peak14.m), peak15.m = peak_moddf$peak15.m, peak16.m = mean(peak_moddf$peak16.m))
# 
# pp.peak15 <- as.data.frame(predict(cv, newx = d_peak15, type = "response", s = "lambda.min"))
# pp.peak15 <- cbind(pp.peak15, peak_moddf$peak15.m)
# 
# pp.peak15 <- pp.peak15%>%
#   transmute(peak = "15", `peak_moddf$peak15.m`, IMM.1, ND.1, DEV.1, VIT.1, PSWN.1, SWN.1, PSNT.1, SNT.1)%>%
#   rename(peak_diff = `peak_moddf$peak15.m`, IMM = IMM.1, ND = ND.1, DEV = DEV.1, VIT = VIT.1, PSWN = PSWN.1, SWN = SWN.1, PSNT = PSNT.1, SNT = SNT.1) #peak diff from mean because centered
# 
# #Peak 16
# d_peak16 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = mean(peak_moddf$peak4.m),  peak5.m = mean(peak_moddf$peak5.m), peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = mean(peak_moddf$peak13.m), peak14.m = mean(peak_moddf$peak14.m), peak15.m = mean(peak_moddf$peak15.m), peak16.m = peak_moddf$peak16.m)
# 
# pp.peak16 <- as.data.frame(predict(cv, newx = d_peak16, type = "response", s = "lambda.min"))
# pp.peak16 <- cbind(pp.peak16, peak_moddf$peak16.m)
# 
# pp.peak16 <- pp.peak16%>%
#   transmute(peak = "16", `peak_moddf$peak16.m`, IMM.1, ND.1, DEV.1, VIT.1, PSWN.1, SWN.1, PSNT.1, SNT.1)%>%
#   rename(peak_diff = `peak_moddf$peak16.m`, IMM = IMM.1, ND = ND.1, DEV = DEV.1, VIT = VIT.1, PSWN = PSWN.1, SWN = SWN.1, PSNT = PSNT.1, SNT = SNT.1) #peak diff from mean because centered


# combine probabilities into one data set
pp.all <- rbind(pp.peak1, pp.peak2, pp.peak3, pp.peak4, pp.peak5, pp.peak6, pp.peak7, pp.peak8, pp.peak9, pp.peak10, pp.peak11, pp.peak12, pp.peak13)

#melt data to long for ggplot
pp.all_l <- melt(pp.all, id.vars = c("peak", "peak_diff"), value.name = "probability")

pp.all_l$peak <- as.numeric(pp.all_l$peak)
...
### Look at predictions and classification
...
ggsave("histosub_fppeaks.pdf", width = 6, height = 5)
...
## Do this again but include length (and maybe GSI)
```{r}
peak_moddf <- peakdat_w[,c(2,21:35)]
peak_mat <- model.matrix(HistoMaturity~., peak_moddf)[,-1]
str(peak_mat)
hist_vec <- factor(peak_moddf$HistoMaturity)
# 
...
#First, find good lambda value for shrinkage. Defined as the lambda that minimizes the cross-validated prediction error rate.
# SEE: https://garthtarr.github.io/avfs/lab03_soln.html
set.seed(13)
cv <- cv.glmnet(x = peak_mat, y = hist_vec, family = "multinomial", alpha = 0) #bootstrap
plot(cv)
...
coefs_df <- pivot_longer(coefs_mat, cols = c(2:16), names_to = "peak_num", values_to = "log_odds")
coefs_df <- coefs_df[,-1]
coefs_df$log_odds <- as.numeric(coefs_df$log_odds)
coefs_df$V17 <- as.factor(coefs_df$V17)
coefs_df$peak_num <- as.factor(coefs_df$peak_num)
...
ggsave("RRR.pdf", width = 4, height = 10)
```
For each explantory variable (peak intensity) null and alternative hypotheses can be formuated as:
H0: The relative risk ratio (odds) associated with explantory variable x is equal to 1.
Ha: The realtive risk ratio (odds) associated wtih the explanatory variable x is not equal to 1. 
...
# Peak 1
d_peak1 <- cbind(peak1.m = peak_moddf$peak1.m, peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = mean(peak_moddf$peak4.m),  peak5.m = mean(peak_moddf$peak5.m), peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = mean(peak_moddf$peak13.m), length.m = mean(peak_moddf$length.m), GSI.m = mean(peak_moddf$GSI.m))

pp.peak1 <- as.data.frame(predict(cv, newx = d_peak1, type = "response", s = "lambda.min", alpha = 0))
pp.peak1 <- cbind(pp.peak1, peak_moddf$peak1.m)
...
pp.peak2 <- as.data.frame(predict(cv, newx = d_peak2, type = "response", s = "lambda.min", alpha = 0))
pp.peak2 <- cbind(pp.peak2, peak_moddf$peak2.m)
...
pp.peak3 <- as.data.frame(predict(cv, newx = d_peak3, type = "response", s = "lambda.min", alpha = 0))
pp.peak3 <- cbind(pp.peak3, peak_moddf$peak3.m)
...
pp.peak4 <- as.data.frame(predict(cv, newx = d_peak4, type = "response", s = "lambda.min", alpha = 0))
pp.peak4 <- cbind(pp.peak4, peak_moddf$peak4.m)
...
pp.peak5 <- as.data.frame(predict(cv, newx = d_peak5, type = "response", s = "lambda.min", alpha = 0))
pp.peak5 <- cbind(pp.peak5, peak_moddf$peak5.m)
...
pp.peak6 <- as.data.frame(predict(cv, newx = d_peak6, type = "response", s = "lambda.min", alpha = 0))
pp.peak6 <- cbind(pp.peak6, peak_moddf$peak6.m)
...
pp.peak7 <- as.data.frame(predict(cv, newx = d_peak7, type = "response", s = "lambda.min", alpha = 0))
pp.peak7 <- cbind(pp.peak7, peak_moddf$peak7.m)
...
pp.peak8 <- as.data.frame(predict(cv, newx = d_peak8, type = "response", s = "lambda.min", alpha = 0))
pp.peak8 <- cbind(pp.peak8, peak_moddf$peak8.m)
...
pp.peak9 <- as.data.frame(predict(cv, newx = d_peak9, type = "response", s = "lambda.min", alpha = 0))
pp.peak9 <- cbind(pp.peak9, peak_moddf$peak9.m)
...
pp.peak10 <- as.data.frame(predict(cv, newx = d_peak10, type = "response", s = "lambda.min", alpha = 0))
pp.peak10 <- cbind(pp.peak10, peak_moddf$peak10.m)
...
pp.peak11 <- as.data.frame(predict(cv, newx = d_peak11, type = "response", s = "lambda.min", alpha = 0))
pp.peak11 <- cbind(pp.peak11, peak_moddf$peak11.m)
...
pp.peak12 <- as.data.frame(predict(cv, newx = d_peak12, type = "response", s = "lambda.min", alpha = 0))
pp.peak12 <- cbind(pp.peak12, peak_moddf$peak12.m)
...
#Peak 13
d_peak13 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = mean(peak_moddf$peak4.m),  peak5.m = mean(peak_moddf$peak5.m), peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = peak_moddf$peak13.m, length.m = mean(peak_moddf$length.m), GSI.m = mean(peak_moddf$GSI.m))

pp.peak13 <- as.data.frame(predict(cv, newx = d_peak13, type = "response", s = "lambda.min", alpha = 0))
pp.peak13 <- cbind(pp.peak13, peak_moddf$peak13.m)
...
# d_peak14 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = mean(peak_moddf$peak4.m),  peak5.m = mean(peak_moddf$peak5.m), peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = mean(peak_moddf$peak13.m), length.m = mean(peak_moddf$length.m), GSI.m = mean(peak_moddf$GSI.m))
# 
# pp.peak14 <- as.data.frame(predict(cv, newx = d_peak14, type = "response", s = "lambda.min", alpha = 0))
# pp.peak14 <- cbind(pp.peak14, peak_moddf$peak14.m)
# 
# pp.peak14 <- pp.peak14%>%
#   transmute(peak = "14", `peak_moddf$peak14.m`, IMM.1, ND.1, DEV.1, VIT.1, PSWN.1, SWN.1, PSNT.1, SNT.1)%>%
...
# 
# #Peak 15
# d_peak15 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = mean(peak_moddf$peak4.m),  peak5.m = mean(peak_moddf$peak5.m), peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = mean(peak_moddf$peak13.m), peak14.m = mean(peak_moddf$peak14.m), peak15.m = peak_moddf$peak15.m, peak16.m = mean(peak_moddf$peak16.m), length.m = mean(peak_moddf$length.m), GSI.m = mean(peak_moddf$GSI.m))
# 
# pp.peak15 <- as.data.frame(predict(cv, newx = d_peak15, type = "response", s = "lambda.min"))
# pp.peak15 <- cbind(pp.peak15, peak_moddf$peak15.m)
# 
...
# 
# #Peak 16
# d_peak16 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = mean(peak_moddf$peak4.m),  peak5.m = mean(peak_moddf$peak5.m), peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = mean(peak_moddf$peak13.m), peak14.m = mean(peak_moddf$peak14.m), peak15.m = mean(peak_moddf$peak15.m), peak16.m = peak_moddf$peak16.m, length.m = mean(peak_moddf$length.m), GSI.m = mean(peak_moddf$GSI.m))
# 
# pp.peak16 <- as.data.frame(predict(cv, newx = d_peak16, type = "response", s = "lambda.min"))
# pp.peak16 <- cbind(pp.peak16, peak_moddf$peak16.m)
# 
...
ggsave("predicted_probs.pdf", width = 7, height = 5)
```
...
ggsave("histosub_peaks_FL_GSI.pdf", width = 6, height = 5)

```

Can use CV model to predict on new dataset!
```{r, include = F}
#Make new dataset
