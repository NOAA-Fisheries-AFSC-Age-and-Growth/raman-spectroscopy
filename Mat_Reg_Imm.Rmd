...
maturity_raw <- read.csv("C:/Users/marri/OneDrive/Documents/AFSC A&G Contract/Walleye Pollock Maturity Data/4Irina.csv", strip.white=TRUE) #load in ancillary data
...
maturity_data$SPECSTAGE <- factor(maturity_data$SPECSTAGE, levels = c("IMM","REG","MAT"))
levels(maturity_data$SPECSTAGE)
...
fp_data_1deriv_w<-fp_data_1deriv%>%
  tidyr::pivot_wider(.,values_from="spc_sg",id_cols=c("filenames", "SPECSTAGE"), names_from="Ramanshift")


m1 = pca(fp_data_1deriv_w[,-c(1:2)], 7, scale = TRUE, info = "Maturity PCA")
...
f <- ggplot(fingerprint_data)+
  geom_line(aes(x = Ramanshift, y = spc_sg, group = filenames, color = SPECSTAGE))+
...
summ <- fingerprint_data %>% 
    group_by(Ramanshift, SPECSTAGE) %>%
    summarize(mean_spc = mean(spc_sg), se_spc = sd(spc_sg)/sqrt(n()))

f <- ggplot(summ)+
  geom_line(aes(x = Ramanshift, y = mean_spc, color = SPECSTAGE))+
  geom_ribbon(aes(x = Ramanshift, ymin = mean_spc - se_spc, ymax = mean_spc + se_spc, fill = SPECSTAGE), alpha = 0.3)+
  scale_color_viridis(discrete = TRUE)+
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m1$res$cal$scores[,1], m1$res$cal$scores[,2], color = fp_data_1deriv_w$SPECSTAGE))+
  labs(title="Scores",
...
=======
t
>>>>>>> 9cca56f2066d5f70c660b7ca1e6edf3b3512b90d
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m1$res$cal$scores[,1], m1$res$cal$scores[,3], color = fp_data_1deriv_w$SPECSTAGE))+
  labs(title="Scores",
       caption="Source: MACE 2017 Pollock",
       x="Comp 1",
       y="Comp 3",
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m1$res$cal$scores[,2], m1$res$cal$scores[,3], color = fp_data_1deriv_w$SPECSTAGE))+
  labs(title="Scores",
       caption="Source: MACE 2017 Pollock",
       x="Comp 1",
       y="Comp 3",
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m2$res$cal$scores[,1], m2$res$cal$scores[,2], color = stretch_data_1deriv_w$SPECSTAGE))+
    scale_color_viridis(discrete = TRUE)+
  labs(title="Scores",
       caption="Source: MACE 2017 Pollock",
       x="Comp 1",
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m2$res$cal$scores[,1], m2$res$cal$scores[,3], color = stretch_data_1deriv_w$SPECSTAGE))+
    scale_color_viridis(discrete = TRUE)+
  labs(title="Scores",
       caption="Source: MACE 2017 Pollock",
       x="Comp 1",
...
t <- ggplotly(ggplot(maturity_data, aes(label = barcode))+
  geom_point(aes(m2$res$cal$scores[,2], m2$res$cal$scores[,3], color = stretch_data_1deriv_w$SPECSTAGE))+
    scale_color_viridis(discrete = TRUE)+
  labs(title="Scores",
       caption="Source: MACE 2017 Pollock",
       x="Comp 1",
