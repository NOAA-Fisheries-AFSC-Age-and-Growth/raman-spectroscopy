...
#Reshape for mlogit()
mdata <- mlogit.data(peakdat_w, varying = NULL, choice = "HistoMaturity", shape = "wide")

# Run a model!
test_mod <- mlogit(HistoMaturity ~ 1 | peak1.m + peak2.m + peak3.m + peak4.m + peak5.m + peak6.m + peak8.m, data = mdata, reflevel = "IMM")

test_mod2 <- multinom(HistoMaturity ~ peak1.m + peak2.m + peak3.m + peak4.m + peak5.m + peak6.m + peak8.m, data = peakdat_w)
...
peak <- rep(c(NA,1,2,3,4,5,6,8), each = 8)
status <- rep(c("DEV", "ND", "OSNT", "PSNT", "PSWN", "SNT", "SWN", "VIT"), times = 8)
RRR_df <- as.data.frame(cbind(RRR, peak, status))

#Prepare data for plotting
RRR_df$peak <- factor(RRR_df$peak)
RRR_df$status <- factor(RRR_df$status, levels = c("OSNT","SNT","PSNT","SWN","PSWN","VIT","DEV","ND","IMM"))
RRR_df$RRR_m <- as.numeric(RRR_df$RRR_m)
RRR_df$RRR_l <- as.numeric(RRR_df$RRR_l)
RRR_df$RRR_u <- as.numeric(RRR_df$RRR_u)

#Try a plot
oo_cols <- length(unique(peakdat_w$HistoMaturity))
...
plot_5 <- ggplot(subset(RRR_df, peak == "5"))+
  geom_pointrange(aes(x = RRR_m, xmin = RRR_l, xmax = RRR_u, y = peak, color = status), position = position_dodge(width=1), key_glyph = "point")+
  xlim(.9, 1.1)+
  scale_color_manual(values = oo_colors)+
  guides(color = guide_legend(reverse = TRUE, override.aes = list(size=2)))+
...
plot_6 <- ggplot(subset(RRR_df, peak == "6"))+
  geom_pointrange(aes(x = RRR_m, xmin = RRR_l, xmax = RRR_u, y = peak, color = status), position = position_dodge(width=1), key_glyph = "point")+
  xlim(.9, 1.1)+
  scale_color_manual(values = oo_colors)+
  guides(color = guide_legend(reverse = TRUE, override.aes = list(size=2)))+
...
plot_7 <- ggplot(subset(RRR_df, peak == "8"))+
  geom_pointrange(aes(x = RRR_m, xmin = RRR_l, xmax = RRR_u, y = peak, color = status), position = position_dodge(width=1), key_glyph = "point")+
  xlim(.9, 1.1)+
  scale_color_manual(values = oo_colors)+
  guides(color = guide_legend(reverse = TRUE, override.aes = list(size=2)))+
...
# Peak 1
d_peak1 <- data.frame(barcode = peakdat_w$barcode, HistoMaturity = peakdat_w$HistoMaturity, peak1.m = peakdat_w$peak1.m, peak2.m = mean(peakdat_w$peak2.m), peak3.m = mean(peakdat_w$peak3.m), peak4.m = mean(peakdat_w$peak4.m), peak5.m = mean(peakdat_w$peak5.m), peak6.m = mean(peakdat_w$peak6.m), peak8.m = mean(peakdat_w$peak8.m))

d_peak1_test <- mlogit.data(d_peak1, varying = NULL, choice = "HistoMaturity", shape = "wide")

pp.peak1 <- as.data.frame(cbind(peakdat_w$peak1.m, predict(test_mod, newdata = d_peak1_test)))

pp.peak1 <- pp.peak1%>%
  transmute(peak = "1", V1, IMM, ND, DEV, VIT, PSWN, SWN, PSNT, SNT, OSNT)%>%
