---
title: "W. pollock maturity stage - ordinal - spectral orange peak height"
...
library(reshape2)
library(caret)
#install.packages("glmnetcr") #https://cran.r-project.org/web/packages/glmnetcr/vignettes/glmnetcr.pdf
# library(glmnetcr) - maybe
install.packages("orindalNet")
library(ordinalNet)
...
ggplot(dat)+
  geom_boxplot(aes(x = HistoMaturity, y = GSI, color = HistoMaturity, fill = HistoMaturity))+
  scale_color_manual(values = oo_colors)+
  scale_fill_manual(values = oo_colors)

ggplot(dat)+
  geom_boxplot(aes(x = HistoMaturity, y = length, color = HistoMaturity, fill = HistoMaturity))+
  scale_color_manual(values = oo_colors)+
  scale_fill_manual(values = oo_colors)
...
Let's try a ridge ordinal regression to deal with multicollinearity. All predictors on same scale already, but will mean center. (Dormann et al.)
```{r, warnings = F, fig.width=6}
...
str(peak_mat)
hist_vec <- factor(peak_moddf$HistoMaturity, order = TRUE, levels = c("IMM",  "ND",   "DEV",  "VIT",  "PSWN", "SWN",  "PSNT", "SNT"))
# 
...
###############Antiquated
#https://cran.r-project.org/web/packages/glmnetcr/glmnetcr.pdf
...
#Helpful figure:http://web.pdx.edu/~newsomj/cdaclass/ho_ordinal%20regression.pdf#:~:text=In%20SPSS%2C%20SAS%2C%20and%20R%2C%20ordinal%20logit%20analysis,accounted%20for%20in%20the%20outcome%20by%20the%20predictors.

#First, explore whether parallel, non-parallel, or semi-parallel provides better model fits
set.seed(13)
fit1_cv <- ordinalNetCV(x = peak_mat, y = hist_vec, family="cumulative", link="logit", parallelTerms=TRUE, nonparallelTerms=FALSE, printProgress=FALSE)

set.seed(13)
fit2_cv <- ordinalNetCV(x = peak_mat, y = hist_vec, family="cumulative", link="logit", parallelTerms=TRUE, nonparallelTerms=TRUE, warn=FALSE, printProgress=FALSE, maxiterOut=200)

set.seed(13)
fit3_cv <- ordinalNetCV(x = peak_mat, y = hist_vec, family="cumulative", link="logit", parallelTerms=FALSE, nonparallelTerms=TRUE, warn=FALSE, printProgress = FALSE)

loglik123 <- cbind(fit1_cv$loglik, fit2_cv$loglik, fit3_cv$loglik)

bestLambdaIndex123 <- cbind(fit1_cv$bestLambdaIndex, fit2_cv$bestLambdaIndex,fit3_cv$bestLambdaIndex)

colnames(loglik123) <- colnames(bestLambdaIndex123) <- c("fit1_cv", "fit2_cv", "fit3_cv")

loglik123 # Best support for parallel model - cool!

# Second, find good λ/model value for shrinkage. Defined as the λ that minimizes the cross-validated prediction error rate
set.seed(13)
fit2_tune <- ordinalNetTune(x = peak_mat, y = hist_vec, family="cumulative", link="logit", parallelTerms=TRUE, nonparallelTerms=FALSE, warn=FALSE, printProgress=FALSE)# The user can this function to tune the model, for example by selecting the λ value with the best average out-of-sample likelihood across folds

bestLambdaIndex <- which.max(rowMeans(fit2_tune$loglik))
...
```{r}
coefs <- coef(fit2_tune$fit, whichLambda = bestLambdaIndex, matrix=TRUE)
coefs_mat <- rbind(coefs[-1,1], coefs[-1,2], coefs[-1,3], coefs[-1,4], coefs[-1,5], coefs[-1,6], coefs[-1,7]) #extract all coefficients - no coefs given for "IMM"
coefs_mat <- as.data.frame(cbind(coefs_mat, c("ND", "DEV", "VIT", "PSWN", "SWN", "PSNT", "SNT")))

coefs_df <- pivot_longer(coefs_mat, cols = c(1:13), names_to = "peak_num", values_to = "log_odds")
coefs_df$log_odds <- as.numeric(coefs_df$log_odds)
coefs_df$V14 <- as.factor(coefs_df$V14)
...
pp.peak1 <- as.data.frame(predict(fit2_tune$fit, newx = d_peak1, type = "response", whichLambda = bestLambdaIndex))
pp.peak1 <- cbind(pp.peak1, peak_moddf$peak1.m)

pp.peak1 <- pp.peak1%>%
  transmute(peak = "1", `peak_moddf$peak1.m`, `P[Y=1]`, `P[Y=2]`, `P[Y=3]`, `P[Y=4]`, `P[Y=5]`, `P[Y=6]`, `P[Y=7]`, `P[Y=8]`)%>%
  rename(peak_diff = `peak_moddf$peak1.m`, IMM = `P[Y=1]`, ND = `P[Y=2]`, DEV = `P[Y=3]`, VIT = `P[Y=4]`, PSWN = `P[Y=5]`, SWN = `P[Y=6]`, PSNT = `P[Y=7]`, SNT = `P[Y=8]`) #peak diff from mean because centered
...
pp.peak2 <- as.data.frame(predict(fit2_tune$fit, newx = d_peak2, type = "response", whichLambda = bestLambdaIndex))
pp.peak2 <- cbind(pp.peak2, peak_moddf$peak2.m)

pp.peak2 <- pp.peak2%>%
  transmute(peak = "2", `peak_moddf$peak2.m`, `P[Y=1]`, `P[Y=2]`, `P[Y=3]`, `P[Y=4]`, `P[Y=5]`, `P[Y=6]`, `P[Y=7]`, `P[Y=8]`)%>%
  rename(peak_diff = `peak_moddf$peak2.m`, IMM = `P[Y=1]`, ND = `P[Y=2]`, DEV = `P[Y=3]`, VIT = `P[Y=4]`, PSWN = `P[Y=5]`, SWN = `P[Y=6]`, PSNT = `P[Y=7]`, SNT = `P[Y=8]`)

# Peak 3
d_peak3 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = peak_moddf$peak3.m, peak4.m = mean(peak_moddf$peak4.m), peak5.m = mean(peak_moddf$peak5.m), peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = mean(peak_moddf$peak13.m))

pp.peak3 <- as.data.frame(predict(fit2_tune$fit, newx = d_peak3, type = "response", whichLambda = bestLambdaIndex))
pp.peak3 <- cbind(pp.peak3, peak_moddf$peak3.m)

pp.peak3 <- pp.peak3%>%
  transmute(peak = "3", `peak_moddf$peak3.m`, `P[Y=1]`, `P[Y=2]`, `P[Y=3]`, `P[Y=4]`, `P[Y=5]`, `P[Y=6]`, `P[Y=7]`, `P[Y=8]`)%>%
  rename(peak_diff = `peak_moddf$peak3.m`, IMM = `P[Y=1]`, ND = `P[Y=2]`, DEV = `P[Y=3]`, VIT = `P[Y=4]`, PSWN = `P[Y=5]`, SWN = `P[Y=6]`, PSNT = `P[Y=7]`, SNT = `P[Y=8]`)

#Peak 4
d_peak4 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = peak_moddf$peak4.m, peak5.m = mean(peak_moddf$peak5.m), peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = mean(peak_moddf$peak13.m))

pp.peak4 <- as.data.frame(predict(fit2_tune$fit, newx = d_peak4, type = "response", whichLambda = bestLambdaIndex))
pp.peak4 <- cbind(pp.peak4, peak_moddf$peak4.m)

pp.peak4 <- pp.peak4%>%
  transmute(peak = "4", `peak_moddf$peak4.m`, `P[Y=1]`, `P[Y=2]`, `P[Y=3]`, `P[Y=4]`, `P[Y=5]`, `P[Y=6]`, `P[Y=7]`, `P[Y=8]`)%>%
  rename(peak_diff = `peak_moddf$peak4.m`, IMM = `P[Y=1]`, ND = `P[Y=2]`, DEV = `P[Y=3]`, VIT = `P[Y=4]`, PSWN = `P[Y=5]`, SWN = `P[Y=6]`, PSNT = `P[Y=7]`, SNT = `P[Y=8]`)

#Peak 5
d_peak5 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = mean(peak_moddf$peak4.m), peak5.m = peak_moddf$peak5.m, peak6.m = mean(peak_moddf$peak6.m), peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = mean(peak_moddf$peak13.m))

pp.peak5 <- as.data.frame(predict(fit2_tune$fit, newx = d_peak5, type = "response", whichLambda = bestLambdaIndex))
pp.peak5 <- cbind(pp.peak5, peak_moddf$peak5.m)

pp.peak5 <- pp.peak5%>%
  transmute(peak = "5", `peak_moddf$peak5.m`, `P[Y=1]`, `P[Y=2]`, `P[Y=3]`, `P[Y=4]`, `P[Y=5]`, `P[Y=6]`, `P[Y=7]`, `P[Y=8]`)%>%
  rename(peak_diff = `peak_moddf$peak5.m`, IMM = `P[Y=1]`, ND = `P[Y=2]`, DEV = `P[Y=3]`, VIT = `P[Y=4]`, PSWN = `P[Y=5]`, SWN = `P[Y=6]`, PSNT = `P[Y=7]`, SNT = `P[Y=8]`)

#Peak 6
d_peak6 <- cbind(peak1.m = mean(peak_moddf$peak1.m), peak2.m = mean(peak_moddf$peak2.m), peak3.m = mean(peak_moddf$peak3.m), peak4.m = mean(peak_moddf$peak4.m), peak5.m = mean(peak_moddf$peak5.m), peak6.m = peak_moddf$peak6.m, peak7.m = mean(peak_moddf$peak7.m), peak8.m = mean(peak_moddf$peak8.m), peak9.m = mean(peak_moddf$peak9.m), peak10.m = mean(peak_moddf$peak10.m), peak11.m = mean(peak_moddf$peak11.m), peak12.m = mean(peak_moddf$peak12.m), peak13.m = mean(peak_moddf$peak13.m))

pp.peak6 <- as.data.frame(predict(fit2_tune$fit, newx = d_peak6, type = "response", whichLambda = bestLambdaIndex))
pp.peak6 <- cbind(pp.peak6, peak_moddf$peak6.m)

pp.peak6 <- pp.peak6%>%
  transmute(peak = "6", `peak_moddf$peak6.m`, `P[Y=1]`, `P[Y=2]`, `P[Y=3]`, `P[Y=4]`, `P[Y=5]`, `P[Y=6]`, `P[Y=7]`, `P[Y=8]`)%>%
  rename(peak_diff = `peak_moddf$peak6.m`, IMM = `P[Y=1]`, ND = `P[Y=2]`, DEV = `P[Y=3]`, VIT = `P[Y=4]`, PSWN = `P[Y=5]`, SWN = `P[Y=6]`, PSNT = `P[Y=7]`, SNT = `P[Y=8]`)
