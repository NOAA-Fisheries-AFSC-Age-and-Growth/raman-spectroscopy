...
ggplot(peak_dat)+
  geom_line(data = fingerprint_data, aes(x = Ramanshift, y = spc_sg, group = filenames))+
  geom_point(aes(ramanshift, peakintensity, group = barcode, color = sscat))+
  theme_classic()
...
#Make plot
ggplot(peak_dat)+
  geom_point(aes(ramanshift, avg_intensity, group = barcode, color = sscat))+
  theme_classic()
```
...
## Pivot wider
peakdat_w<-peak_dat%>%
  tidyr::pivot_wider(.,values_from=c("peakintensity"),id_cols=c("barcode", "sscat", "GSI", "length"), names_from="peaknum")

ggplot(peakdat_w)+
  geom_bar(aes(peakdat_w$sscat))+
  theme_classic()
```
...
peak_moddf <- peakdat_w[,c(2,15:22)]
